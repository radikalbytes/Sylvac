...
def process_measurement(_, data, real_time_label):
    ...
    # Convertir la medición de micras a milímetros (1 mm = 1,000,000 micras)
    latest_measurement = measurement / 1000000.0  # Dividir por 1,000,000 para pasar de micras a milímetros
...
async def take_measurements(client, listbox, status_label, real_time_label, fig, ax, canvas):
    ...
    try:
        ...
        while measurement_count < max_measurements:
            ...
            # Redondear la medición a 6 decimales
...
def start_measurements(entry_count, entry_interval, listbox, status_label, real_time_label, fig, ax, canvas):
    global max_measurements, interval_seconds, measurement_count, times_data, measurements_data

    try:
        max_measurements = int(entry_count.get())
        interval_seconds = float(entry_interval.get())/1000.0

        # Limpiar el Listbox y la gráfica
...
def create_window():
    ...
    tk.Label(window, text="Intervalo entre mediciones (ms > 100):").pack()
    ...
    real_time_label = tk.Label(window, text="Valor en tiempo real: --- mm", font=("Helvetica", 12))
    ...
    button_start = tk.Button(window, text="Iniciar Mediciones", command=lambda: start_measurements(entry_count, entry_interval, listbox, status_label, real_time_label, fig, ax, canvas))
